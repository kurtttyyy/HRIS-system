...
<head>
  ...
  <style>
    ...
    @media (max-width: 767px) {
      ...
      #teaching-matrix td:nth-child(2)::before { content: "Education"; }
      #teaching-matrix td:nth-child(3)::before { content: "Specialization"; }
      #teaching-matrix td:nth-child(4)::before { content: "Subject assignments"; }
      #teaching-matrix td:nth-child(5)::before { content: "Employment status"; }
      #teaching-matrix td:nth-child(6)::before { content: "Weekly contact hours"; }
      #teaching-matrix td:nth-child(7)::before { content: "Salary rate"; }
      #teaching-matrix td:nth-child(8)::before { content: "Benefits"; }
      #teaching-matrix td:nth-child(9)::before { content: "Experience"; }
        ...
        <table id="teaching-matrix" class="min-w-[1700px] w-full text-sm text-stone-800 border-collapse">
          <thead class="bg-stone-100">
            <tr class="hidden print:table-row">
              <th colspan="9" class="border border-stone-900 bg-white px-3 py-2 text-left text-sm font-bold text-stone-900">
          ...
          <tbody class="align-top">
            @forelse ($rows as $staff)
              @php
                ...
                $existingSubjectLoads = collect(data_get($staff, 'subject_loads', []))
                  ->map(function ($row) {
                    $subjectName = trim((string) data_get($row, 'subject_name'));
                    $code = trim((string) data_get($row, 'code'));
                    $courseNo = trim((string) data_get($row, 'course_no'));
                    $units = trim((string) data_get($row, 'units'));

                    $subjectLabel = $subjectName !== ''
                      ? $subjectName
                      : collect([$code, $courseNo])->filter()->implode(' ');
                    ...
                    return $subjectLabel.' ('.$units.' units)';
                  })
                  ->filter()
                  ->values();

                $subjectLoads = $existingSubjectLoads->isNotEmpty()
                  ? $existingSubjectLoads
                  : collect([
                      ...
                      ->reject(function ($value) {
                        ...
                        return in_array($normalized, ['a', 'n/a', 'na'], true);
                      })
                      ->values();
