          ...
          <tbody class="align-top">
            @forelse ($administrators as $admin)
              @php
                ...
                $middleNameDisplay = trim((string) ($admin->middle_name ?? '')) !== '' ? trim((string) ($admin->middle_name ?? '')) : 'N/A';
                $accountNumberDisplay = trim((string) (optional($admin->employee)->account_number ?? '')) !== '' ? trim((string) (optional($admin->employee)->account_number ?? '')) : 'N/A';
                $civilStatusDisplay = trim((string) (optional($admin->employee)->civil_status ?? '')) !== '' ? trim((string) (optional($admin->employee)->civil_status ?? '')) : 'N/A';
                $contactNumberDisplay = collect([
                  trim((string) (optional($admin->employee)->contact_number ?? '')),
                  trim((string) (optional($admin->applicant)->phone ?? '')),
                ])->first(fn ($value) => $value !== '') ?? 'N/A';
                $birthdayDisplay = 'N/A';
                if (!empty(optional($admin->employee)->birthday)) {
                  try {
                    $birthdayDisplay = \Carbon\Carbon::parse(optional($admin->employee)->birthday)->format('F j, Y');
                  } catch (\Throwable $e) {
                    $birthdayDisplay = trim((string) (optional($admin->employee)->birthday ?? '')) !== '' ? trim((string) (optional($admin->employee)->birthday ?? '')) : 'N/A';
                  }
                }
                $addressDisplay = collect([
                  trim((string) (optional($admin->employee)->address ?? '')),
                  trim((string) (optional($admin->applicant)->address ?? '')),
                ])->first(fn ($value) => $value !== '') ?? 'N/A';
                $classificationDisplay = collect([
                  trim((string) (optional($admin->employee)->classification ?? '')),
                  trim((string) (optional(optional($admin->applicant)->position)->employment ?? '')),
                  trim((string) (optional($admin->employee)->job_type ?? '')),
                ])->first(fn ($value) => $value !== '') ?? 'N/A';
                $licenseDisplay = trim((string) (optional($admin->license)->license ?? '')) !== '' ? trim((string) (optional($admin->license)->license ?? '')) : 'N/A';
                $registrationNumberDisplay = trim((string) (optional($admin->license)->registration_number ?? '')) !== '' ? trim((string) (optional($admin->license)->registration_number ?? '')) : 'N/A';
                $registrationDateDisplay = 'N/A';
                if (!empty(optional($admin->license)->registration_date)) {
                  try {
                    $registrationDateDisplay = \Carbon\Carbon::parse(optional($admin->license)->registration_date)->format('F j, Y');
                  } catch (\Throwable $e) {
                    $registrationDateDisplay = trim((string) (optional($admin->license)->registration_date ?? '')) !== '' ? trim((string) (optional($admin->license)->registration_date ?? '')) : 'N/A';
                  }
                }
                $validUntilDisplay = 'N/A';
                if (!empty(optional($admin->license)->valid_until)) {
                  try {
                    $validUntilDisplay = \Carbon\Carbon::parse(optional($admin->license)->valid_until)->format('F j, Y');
                  } catch (\Throwable $e) {
                    $validUntilDisplay = trim((string) (optional($admin->license)->valid_until ?? '')) !== '' ? trim((string) (optional($admin->license)->valid_until ?? '')) : 'N/A';
                  }
                }
                $bachelorDisplay = trim((string) (optional($admin->education)->bachelor ?? '')) !== '' ? trim((string) (optional($admin->education)->bachelor ?? '')) : 'N/A';
                $masterDisplay = trim((string) (optional($admin->education)->master ?? '')) !== '' ? trim((string) (optional($admin->education)->master ?? '')) : 'N/A';
                $doctorateDisplay = trim((string) (optional($admin->education)->doctorate ?? '')) !== '' ? trim((string) (optional($admin->education)->doctorate ?? '')) : 'N/A';
                $salaryDisplay = trim((string) (optional($admin->salary)->salary ?? '')) !== '' ? trim((string) (optional($admin->salary)->salary ?? '')) : 'N/A';
                $colaDisplay = trim((string) (optional($admin->salary)->cola ?? '')) !== '' ? trim((string) (optional($admin->salary)->cola ?? '')) : 'N/A';
                $sssDisplay = trim((string) (optional($admin->government)->SSS ?? '')) !== '' ? trim((string) (optional($admin->government)->SSS ?? '')) : 'N/A';
                $tinDisplay = trim((string) (optional($admin->government)->TIN ?? '')) !== '' ? trim((string) (optional($admin->government)->TIN ?? '')) : 'N/A';
                $philHealthDisplay = trim((string) (optional($admin->government)->PhilHealth ?? '')) !== '' ? trim((string) (optional($admin->government)->PhilHealth ?? '')) : 'N/A';
                $pagIbigMidDisplay = trim((string) (optional($admin->government)->MID ?? '')) !== '' ? trim((string) (optional($admin->government)->MID ?? '')) : 'N/A';
                $pagIbigRtnDisplay = trim((string) (optional($admin->government)->RTN ?? '')) !== '' ? trim((string) (optional($admin->government)->RTN ?? '')) : 'N/A';
